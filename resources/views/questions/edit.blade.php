...
@section('content')
<div class="mb-4"><h2><i class="bi bi-pencil-square me-2"></i>Sửa Câu hỏi</h2></div>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select name="topic_id" required class="form-select">
                            ...
                            @foreach($topics as $topic)<option value="{{ $topic->id }}" {{ old('topic_id', $question->topic_id) == $topic->id ? 'selected' : '' }}>{{ $topic->name }}</option>@endforeach
                        </select>
                    </div>
                    <div class="row">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="content" rows="4" required class="form-control">{{ old('content', $question->content) }}</textarea>
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="form-check"><input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $question->is_active) ? 'checked' : '' }} class="form-check-input"><label for="is_active" class="form-check-label">Hoạt động</label></div>
                </div>
            </div>
            <div class="card shadow-sm mb-4">
        ...
        <div class="col-lg-4">
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body small text-muted">
                    ...
                    <p><i class="bi bi-check-circle text-info me-1"></i> <strong>Nhiều lựa chọn:</strong> Nhiều đáp án đúng</p>
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body d-grid gap-2">
                    ...
                    <a href="{{ route('questions.show', $question) }}" class="btn btn-outline-secondary">Hủy bỏ</a>
                </div>
            </div>
        </div>
    </div>
...
function updateHint() {
    const type = document.getElementById('typeSelect').value;
    document.getElementById('answerHint').classList.remove('d-none');
...
@endforeach
if (answerIndex === 0) { addAnswerRow(); addAnswerRow(); }
...
document.getElementById('answersContainer').addEventListener('click', (e) => {
    ...
    if (btn) { if (document.getElementById('answersContainer').children.length > 2) btn.closest('.answer-row').remove(); else alert('Phải có ít nhất 2 đáp án!'); }
});
