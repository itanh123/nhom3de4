...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-800">Sửa Câu hỏi</h1>
</div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="type" id="typeSelect" required class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 @error('type') border-red-500 @enderror">
                                <option value="single_choice" {{ old('type', $question->type) == 'single_choice' ? 'selected' : '' }}>Một lựa chọn</option>
                                <option value="multiple_choice" {{ old('type', $question->type) == 'multiple_choice' ? 'selected' : '' }}>Nhiều lựa chọn</option>
                                <option value="fill_in_blank" {{ old('type', $question->type) == 'fill_in_blank' ? 'selected' : '' }}>Điền trống</option>
                            </select>
                            @error('type')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea name="content" rows="4" required class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 @error('content') border-red-500 @enderror" placeholder="Nhập nội dung câu hỏi...">{{ old('content', $question->content) }}</textarea>
                        @error('content')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="explanation" rows="3" class="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 @error('explanation') border-red-500 @enderror" placeholder="Giải thích đáp án đúng...">{{ old('explanation', $question->explanation) }}</textarea>
                        @error('explanation')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $question->is_active) ? 'checked' : '' }} class="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6">
                ...
                <div id="answersContainer" class="space-y-3">
                    <!-- Answer rows will be loaded here -->
                </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6">
                ...
                <div class="space-y-3">
                    ...
                    <a href="{{ route('questions.show', $question) }}" class="block w-full px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl hover:bg-slate-200 transition-colors font-medium text-center">
                        Hủy bỏ
                    </a>
                </div>
            </div>
...
addAnswerRow('{{ addslashes($answer->option_text) }}', {{ $answer->is_correct ? 'true' : 'false' }});
@endforeach

// If no answers exist, add 2 empty rows
if (answerIndex === 0) {
    addAnswerRow();
    addAnswerRow();
}
